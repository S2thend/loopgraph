 这份记录保留了 loop re-entry 设计讨论；下面引用的代码路径已按当前仓库结构更新。
...
• Explored
  └ Read research.md, plan.md, scheduler.py, state.py
    ...
    Search class ExecutionState|def is_ready|def mark_complete|def mark_running in loopgraph
...
• 你这个追问很关键。给你基于现有代码证据的背景：
  ...
  - 但 scheduler 目前“只跑一次节点”：
      - run() 初始化 pending 后循环执行，节点完成后直接 pending.remove(node_id)（../loopgraph/scheduler/scheduler.py:184）。
      - 节点完成会被标记为 COMPLETED（../loopgraph/scheduler/scheduler.py:246，../loopgraph/core/state.py:300）。
      - is_ready() 只有 PENDING 才可调度（../loopgraph/core/state.py:196）。
  - 虽然 SWITCH 能选到回边（../loopgraph/scheduler/scheduler.py:357-364），但当前逻辑只 note_upstream_completion，不会把目
    标节点重新放回 pending（../loopgraph/scheduler/scheduler.py:272）。
  - 现有 loopgraph 测试也在“手工重置状态”来模拟 loop（../tests/test_integration_workflows.py:176-186），反向证明调度器没原


  - 改点就在 scheduler：
      - 在 _execute_node 选出 downstream edges 后（../loopgraph/scheduler/scheduler.py:260-267），对命中的回边目标做 re-
        entry：
  ...
  - 所以 plan 才给了“改 loopgraph 是最小可控改动”的结论（specs/010-unified-graph-loop/research.md:116-125）。

  基于这些背景，C2 你选哪个：

  - B：不改 loopgraph，改成 core workaround

› A 而且我们还需要在loopgraph支持回环次数（PENDING）


■ Conversation interrupted - tell the model what to do differently. Something went wrong? Hit `/feedback` to report the issue.


› A 而且我们还需要在loopgraph支持回环次数（PENDING重置次数）允许completed被调度时重回PENDING


• 收到，记录 C2 结论：

  - 选 A（允许 loopgraph 源码改动，走显式审批）
  - 你补充的强约束也记录：
...
#### 1. `loopgraph/core/state.py` — 新增 `reset_for_reentry()`
...
#### 2. `loopgraph/scheduler/scheduler.py` — 两处改动
